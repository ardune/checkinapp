...
{
    [FunctionName("Broke")]
    public static async Task<IActionResult> ReportBroke([HttpTrigger(AuthorizationLevel.Function, "post", Route = "broke")] HttpRequest req, ILogger log)
    {
        log.LogWarning(await req.ReadAsStringAsync());
        return new OkResult();
    }
    ...
    public static async Task<IActionResult> ReportSentSms([HttpTrigger(AuthorizationLevel.Function, "post", Route = "sms-out")] HttpRequest req, ILogger log)
    {
        ...
        var status = form["MessageStatus"];
        if (!string.Equals(status, "delivered"))
        {
            return new OkResult();
        }
